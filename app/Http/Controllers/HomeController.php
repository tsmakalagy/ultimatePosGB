        ...
        $datatable = Datatables::of($sells)
            ...
            function ($row) {
              ...
              $test='';
               ...
               // $test= '<span class="date_paid_on">{{$transaction_payments}}</span>' ;
                /* for($i=0;$i<3;$i++){ }*/
                //return '<span class="date_paid_on">'.$this->transactionUtil->getPaidOn($transaction_payments).'</span>';
               // for($i=0;$i<3;$i++){
               // echo '<span class="date_paid_on"> ujuiuj </span>';
               // }

            /*}) 
               //'<span class="date_paid_on"> @for($i=0;$i<=3;$i++)ujuiuj @endfor</span>')
            ->editColumn(
                'total_before_tax',
          ...
          /*  ->editColumn('shipper_name',
            ...
            ->filterColumn('date_paid_on', function ($query, $keyword) {
                $query->where(function ($q) use ($keyword) {
                    $q->where('tp.paid_on', 'like', "%{$keyword}%");
