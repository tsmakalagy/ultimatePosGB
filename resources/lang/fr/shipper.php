<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Shipper Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used for Shipper CRUD operations.
    |
    */

    'shipper' => 'Livreur',
    'shippers' => 'Livreurs',
    'shipper_name' => 'Livreur',
    'shipping_details' => 'Detail de livraison',
    'name' => 'Nom',
    'type' => 'Type',
    'tel' => 'Tel',
    'other_details' => 'Autre details',
    'details' => 'Detail du livreur',
    'other_detail' => 'Autre detail',
    'shipper_details' => 'Details du livreur',
    'list' => 'Liste des livreurs',
    'add' => 'Ajouter un livreur',
    'all_shippers' => 'Tous les livreurs',
    'shipper_added_successfully' => 'Livreur ajouté avec succès',
    'delete_success' => 'Livreur supprimé avec succès',
    'view_details' => 'Afficher les détails du livreur',
    'shipping_charges' => 'Frais de livraison',
    'shipping_date' => 'Date de livraison',
    'shipping_status' => 'État de livraison',
    'edit_shipper' => 'Modifier le livreur',
];
